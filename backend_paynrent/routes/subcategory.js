...
router.post("/submitsubcategory", upload.any(), function (req, res, next) {
  ...
  pool.query(
    "insert into subcategory(categoryid,subcategoryname,icon,priority) values(?,?,?,?)",
    [
      req.body.categoryid,
  ...
  pool.query(
    ...
    function (error, result) {
      if (error) {
        ...
        res
          .status(200)
          .json({ status: true, message: "SubCategory Updated Successfully" });
      }
    }
  );
});
...
router.get("/display_all_subcategory", function (req, res, next) {
  pool.query(
    "select SC.*,(select categoryname from category C where C.categoryid=SC.categoryid) as cname from subcategory SC",
    function (error, result) {
      if (error) {
        res.status(500).json({ status: false, message: "Server Error" });
      } else {
