...
router.post("/submitvehicle", upload.any(), function (req, res, next) {
  ...
  pool.query(
    "insert into vehicle(categoryid,subcategoryid,companyid,modelid,vendorid,registrationno,color,fueltype,ratings,average,remarks,capacity,status,feature,mode,icon) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    [
      req.body.categoryid,
...
router.post("/edit_picture", upload.single("icon"), function (req, res, next) {
  ...
  pool.query(
    "update vehicle set icon=? where vehicleid=?",
    [req.file.filename, req.body.vehicleid],
    function (error, result) {
      if (error) {
        console.log(error);
        res.status(500).json({ status: false, message: "Server Error" });
        ...
        fs.unlinkSync(
          `${"C:/Users/bhara/backend_paynrent/public/images"}/${
            req.body.oldicon
          }`
        );
        res
...
router.post("/edit_data", upload.single("icon"), function (req, res, next) {
  console.log(req.file);
  pool.query(
    "update vehicle set categoryid=?,subcategoryid=?,companyid=?,modelid=?,vendorid=?,registrationno=?,color=?,fueltype=?,ratings=?,average=?,remarks=?,capacity=?,status=?,feature=?,mode=? where vehicleid=?",
    [
      req.body.categoryid,
      req.body.subcategoryid,
      req.body.companyid,
    ...
    function (error, result) {
      if (error) {
        ...
        res
          .status(200)
          .json({ status: true, message: "Vehicle Updated Successfully" });
      }
    }
  );
});
  ...
  pool.query(
    ...
    function (error, result) {
      if (error) {
        ...
        res
          .status(200)
          .json({ status: true, message: "Vehicle delete Successfully" });
      }
    }
  );
});
