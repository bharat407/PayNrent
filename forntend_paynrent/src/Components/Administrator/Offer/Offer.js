...
export default function Offer(props) {
  ...
  var [icon, setIcon] = useState({ filename: "/assest/Ofr.png", bytes: "" });
  ...
  const handleSubmit = async () => {
    ...
    var response = await postData("offer/offersubmit", formData);
    if (response.status) {
      Swal.fire({
        icon: "success",
        title: "Done",
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} className={classes.headingStyle}>
            <div className={classes.center}>
              ...
              <div style={{ marginLeft: 5 }}>Offer Interface</div>
            </div>
          </Grid>
