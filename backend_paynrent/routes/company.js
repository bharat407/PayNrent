...
router.post("/fetch_all_company_by_subcategory", function (req, res, next) {
  pool.query(
    "select S.*,(select C.subcategoryname from subcategory C where C.subcategoryid=S.subcategoryid) as subcategoryname from company S where S.subcategoryid=?",
...
router.post("/submitcompany", upload.any(), function (req, res, next) {
  ...
  pool.query(
    "insert into company(categoryid,subcategoryid,companyname,icon) values(?,?,?,?)",
    [
      req.body.categoryid,
      req.body.subcategoryid,
...
router.post("/edit_picture", upload.single("icon"), function (req, res, next) {
  ...
  pool.query(
    "update company set icon=? where companyid=?",
...
router.post("/edit_data", upload.single("icon"), function (req, res, next) {
  ...
  pool.query(
    ...
    [req.body.companyname, req.body.companyid],
    function (error, result) {
      if (error) {
        console.log(error);
        res.status(500).json({ status: false, message: "Server Error" });
